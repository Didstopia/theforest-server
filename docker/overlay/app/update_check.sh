...
if [ "$THEFOREST_SERVER_UPDATE_CHECKING" = "1" ]; then
...
OLD_BUILDID=0
if [ -f "/steamcmd/theforest/build.id" ]; then
	OLD_BUILDID="$(cat /steamcmd/theforest/build.id)"
fi

# Minimal validation for the update branch
STRING_SIZE=${#THEFOREST_SERVER_UPDATE_BRANCH}
...
NEW_BUILDID="$(./steamcmd/steamcmd.sh +login anonymous +app_info_update 1 +app_info_print "556450" +quit | grep -EA 1000 "^\s+\"branches\"$" | grep -EA 5 "^\s+\"$THEFOREST_SERVER_UPDATE_BRANCH\"$" | grep -m 1 -EB 10 "^\s+}$" | grep -E "^\s+\"buildid\"\s+" | tr '[:blank:]"' ' ' | tr -s ' ' | sed "s/ buildid //g" | xargs)"
...
if [ ! -f "/steamcmd/theforest/build.id" ]; then
	...
	echo $NEW_BUILDID > /steamcmd/theforest/build.id
	exit
else
...
if [ "$OLD_BUILDID" = "$NEW_BUILDID" ]; then
  ...
  kill -2 $(pgrep -f "TheForestDedicatedServer.exe")
	child=$!
	wait "$child"
fi
